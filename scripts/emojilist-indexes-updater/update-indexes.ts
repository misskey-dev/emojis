...
const enAnnotationsMap = new Map(
    Object.entries(enAnnotations.annotations.annotations),
);
const enAnnotationsDerivedMap = new Map(
    Object.entries(enAnnotationsDerived.annotationsDerived.annotations),
);
const jaAnnotationsMap = new Map(
    Object.entries(jaAnnotations.annotations.annotations),
);
const jaAnnotationsDerivedMap = new Map(
    Object.entries(jaAnnotationsDerived.annotationsDerived.annotations),
);
    ...
    const countryCode = isCountryFlag
        ? String.fromCodePoint(
            ...codePoints.map((codePoint) =>
                codePoint as number - regionalIndicatorACodePoint + aCodePoint,
            ),
        )
        : null;
    ...
    switch (language) {
        case 'en-US': {
            if (isCountryFlag && countryCode != null) {
                ...
                if (
                    annotations && 'default' in annotations &&
            ...
            if (language === 'ja-JP_hira') {
                result = await Promise.all(
                    result.map((word) => {
                        ...
                        return tokenize(word).then((tokens) =>
                            toHiragana(
                                tokens.map(
                                    (token) =>
                                        (token.reading as string | undefined) ?? token.surface_form,
                                ).join(''),
...
export async function updateUnicodeEmojiIndex(
    ...
    const emojiIndex = Object.entries(
        JSON.parse(
            fs.readFileSync(
                `${unicodeEmojiIndexesPath}/${language}.json`,
                { encoding: 'utf-8' },
            ),
        ),
    ) as [string, string[]][];
        ...
        if (
            ...
            if (
                ...
                if (language !== 'en-US' && isCountryFlag && countryCode != null) {
                    while (emojiIndexIndex < emojiIndex.length) {
                        ...
                        if (
                            ...
                            String.fromCodePoint(
                                ...Array.from(emojiIndexEmoji).map((c) =>
                                    c.codePointAt(0) as number - regionalIndicatorACodePoint +
                                    aCodePoint,
                                ),
                            ) > countryCode
                        ) {
                    ...
                    if (emojilistIndex < 0) {
                        emojilistIndex = emojilist.findIndex(
                            (e) => (e[0] as string).replaceAll(vs16, '') === discoloredEmoji,
                        );
                    }
                    while (emojiIndexIndex < emojiIndex.length) {
                        const emojiIndexEmoji = emojiIndex[emojiIndexIndex][0];
                        if (
                            emojilist.findIndex((e) => e[0] === emojiIndexEmoji) >
                            emojilistIndex
                        ) {
                            break;
                        }
                ...
                emojiIndex.splice(
                    emojiIndexIndex,
                    0,
                    [emoji.emoji, await getAnnotations(emoji, language)],
                );
            } else {
                let emojiIndexIndex = emojiIndex.findIndex((e) => e[0] === emoji.emoji);
                if (emojiIndexIndex < 0) {
                    emojiIndexIndex = emojiIndex.findIndex(
                        (e) => e[0].replaceAll(vs16, '') === discoloredEmoji,
                    );
                }
                if (emojiIndexIndex < 0) {
                ...
                if (emojiIndex[emojiIndexIndex][1].length === 0) {
                    emojiIndex.splice(
                        ...
                        1,
                        [
                            emojiIndex[emojiIndexIndex][0],
                            await getAnnotations(emoji, language),
                        ],
                    );
                }
            }
    ...
    fs.writeFileSync(`${unicodeEmojiIndexesPath}/${language}.json`, JSON.stringify(emojiIndex), 'utf-8');
}
